(function () {
  ...
  function render() {
    ...
    var entries = Object.entries(items).filter(function (entry) {
      var slot = entry[1].slot;
      return !filter || (filter === 'legs' ? slot === 'l-leg' || slot === 'r-leg' : slot === filter);
    });
    if (!entries.length) {
  ...
  function loadProfile() {
    ...
    return db.ref('users/' + username).once('value').then(function (snap) {
      ...
      return Object.keys(updates).length ? db.ref('users/' + username).update(updates).then(function () { return user; }) : user;
    });
  }
  ...
  window.filterCategory = function (slot, button) { filter = filter === slot ? null : slot; document.querySelectorAll('.cat-button').forEach(function (b) { b.classList.remove('active'); }); if (filter && button) button.classList.add('active'); render(); };
